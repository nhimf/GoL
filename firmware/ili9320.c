...
void Display_SetPoint_old( uint16_t Xpos, uint16_t Ypos, uint16_t point)
{
...
void Display_SetPoint( uint16_t Xpos, uint16_t Ypos, uint16_t point)
{
	...
	SPI_WriteReadByte(SPI_START | SPI_WR | SPI_DATA);    /* Write : RS = 1, RW = 0       */
	SPI_WriteReadByte((Xpos >>   8));                    /* Write D8..D15                */
	SPI_WriteReadByte((Xpos & 0xFF));                    /* Write D0..D7                 */
	...
	SPI_WriteReadByte(SPI_START | SPI_WR | SPI_DATA);    /* Write : RS = 1, RW = 0       */
	SPI_WriteReadByte((Ypos >>   8));                    /* Write D8..D15                */
	SPI_WriteReadByte((Ypos & 0xFF));                    /* Write D0..D7                 */
	...
	SPI_WriteReadByte(SPI_START | SPI_WR | SPI_DATA);    /* Write : RS = 1, RW = 0       */
	SPI_WriteReadByte((point >>   8));                    /* Write D8..D15                */
	SPI_WriteReadByte((point & 0xFF));                    /* Write D0..D7                 */
...
void Display_DrawFilledRectangle ( uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1 , uint16_t color )
{
    uint32_t index=0;
    uint16_t i;
